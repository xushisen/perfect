$.ts = function(options) {
	...
	var date = opts.date;
	opts.create(width, height, color, content, backgroundColor, type,
			direction, date);
};
$.ts.method = {
	create : function(width, height, color, content, backgroundColor, type,
			direction, date) {
		if (direction == "around") {
			...
			$(".tskdh").css(
					{
						...
						"width" : "" + width + "",
						"height" : "" + height + "",
						"-webkit-animation" : "tckdh" + type
								+ " 1s 1 ease-in-out forwards",
						"animation" : "tckdh" + type
								+ " 1s 1 ease-in-out forwards;"
					});
			setTimeout(function() {
				$(".tskdh").remove();
			...
			if (direction == "among") {
				...
				if (direction == "load") {
					var loadDiv = '<div class="loading"><span>数据加载中...</span></div>';
					$("body").append(loadDiv);
					$(".loading").css({
						"position" : "fixed",
						"top" : "50%",
